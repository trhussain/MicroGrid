...
void disableAntennaSwitching() {
  wifi_ant_gpio_config_t config;

  // Zero out all config to be safe
  memset(&config, 0, sizeof(config));

  // Only configure the first antenna GPIO
  config.gpio_cfg[0].gpio_select = false;
  config.gpio_cfg[0].gpio_num = 0;  // Ignored if gpio_select is false
...
void handleWebSocketMessage(AsyncWebSocketClient *client, void *arg, uint8_t *data, size_t len) {
  ...
  if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
    ...
    } else if (strstr(message, "scary") != NULL) {
      scaryMethod();
    }
  }
}
